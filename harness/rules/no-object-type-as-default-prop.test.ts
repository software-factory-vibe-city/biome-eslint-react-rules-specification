...
describe("no-object-type-as-default-prop", () => {
  describe("valid", () => {
    it("valid[0]: function Foo({ bar = emptyFunction, }) { return null; }", async ({ task }) => {
      const code = `
      function Foo({
        bar = emptyFunction,
      }) {
    ...
    `;
      ...
      const matches = ruleErrors(diags, "no-object-type-as-default-prop", RULE_MESSAGES);
      expect(matches).toHaveLength(0);
    });

    it("valid[1]: function Foo({ bar = emptyFunction, ...rest }) { return n...", async ({ task }) => {
      const code = `
      function Foo({
        bar = emptyFunction,
        ...rest
    ...
    it("valid[2]: function Foo({ bar = 1, baz = 'hello', }) { return null; }", async ({ task }) => {
      const code = `
      function Foo({
        bar = 1,
        baz = 'hello',
    ...
    it("valid[3]: function Foo(props) { return null; }", async ({ task }) => {
      const code = `
      function Foo(props) {
        return null;
      }
    ...
    it("valid[4]: function Foo(props) { return null; } Foo.defaultProps = {...", async ({ task }) => {
      const code = `
      function Foo(props) {
        return null;
      }
    ...
    it("valid[5]: const Foo = () => { return null; };", async ({ task }) => {
      const code = `
      const Foo = () => {
        return null;
      };
    ...
    it("valid[6]: const Foo = ({bar = 1}) => { return null; };", async ({ task }) => {
      const code = `
      const Foo = ({bar = 1}) => {
        return null;
      };
    ...
    it("valid[7]: const Foo = ({bar = 1}, context) => { return null; };", async ({ task }) => {
      const code = `
    ...
    it("valid[8]: export default function NotAComponent({foo = {}}) {}", async ({ task }) => {
      const code = `
  ...
  describe("invalid", () => {
    it("invalid[0]: function Foo({ a = {}, b = ['one', 'two'], c = /regex/i, ...", async ({ task }) => {
      const code = `
        function Foo({
          a = {},
          b = ['one', 'two'],
    ...
    it("invalid[1]: const Foo = ({ a = {}, b = ['one', 'two'], c = /regex/i, ...", async ({ task }) => {
      const code = `
        const Foo = ({
          a = {},
          b = ['one', 'two'],
    ...
    it("invalid[2]: const Foo = ({ a = {}, b = ['one', 'two'], c = /regex/i, ...", async ({ task }) => {
      const code = `
        const Foo = ({
          a = {},
          b = ['one', 'two'],
