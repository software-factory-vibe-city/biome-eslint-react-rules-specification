...
As long as the repository has `.grit` directory (in the root) with at least one [pattern](./patterns.md) defined, it will be recognized as a pattern repository.
...
{% note type="warning" %}
As a reminder, the file name for `.grit` patterns is [not important](./patterns.md#grit-pattern-files). You should import patterns by the name of the pattern, not the file name.
{% /note %}