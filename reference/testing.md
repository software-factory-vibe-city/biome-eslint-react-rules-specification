...
See the [config guide](./config.md#spec) for the full markdown pattern specification.

You can find many more examples in the [Grit standard library](https://github.com/getgrit/stdlib/tree/main/.grit/patterns).

## YAML format

If you define patterns directly in the `.grit/grit.yaml` [file](./config.md#configuration-reference), you can also include before and after test cases underneath the `samples` key. Each sample _must_ include a `input` and `output` key.
